...
// Normalize canonical URLs: force https + smartpixel.in host (non-www), strip trailing
...
const normalizeCanonical = (input: string): string => {
  try {
    ...
    if (path === "") path = "/";
    return `https://smartpixel.in${path}${u.search}`;
  } catch {
    return input;
  }
};
            ...
            return (
              <span key={cleanUrl}>
                ...
                {i === breadcrumbs.length - 1 ? (
                  ...
                  <Link 
                    to={cleanUrl.replace("https://smartpixel.in", "") || "/"} 
